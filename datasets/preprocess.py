...
def pre_normalization(data, center_joint=11, zaxis=[11, 5], xaxis=[]):
    """
    Normalization steps:
...
if __name__ == '__main__':
    data_path = ['../data/train_joint.npy','../data/test_A_joint.npy']
    # data_path = '../data/test_A_joint.npy'
    predata_path = ['../data_prenormalized/train_joint2.npy','../data_prenormalized/test_A_joint2.npy']
    # predata_path = '../data_prenormalized/test_A_joint.npy'
    # data = np.load(data_path)
    # data = pre_normalization(data)
    # np.save(predata_path, data)

    for path,prepath in zip(data_path,predata_path):
        data = np.load(path)
        data = pre_normalization(data)
        np.save(prepath, data)
