...
def conv_branch_init(conv, branches):
    ...
    n = weight.size(0)
    k1 = weight.size(1)
    k2 = weight.size(2) # 分别为卷积核的个数和高宽
    nn.init.normal_(weight, 0, math.sqrt(2. / (n * k1 * k2 * branches))) # 按照正态分布对卷积核权重进行初始化
    nn.init.constant_(conv.bias, 0) # 偏置为0

def conv_init(conv):
    """
    标准的卷积层初始化函数，采用了He初始化
    """
...
def bn_init(bn, scale):
    """
    初始化批量归一化（Batch Normalization, BN）层
    :param bn: 需要初始化的bn层
    :param scale: 对 bn.weight 进行初始化的缩放系数
    """
...
class unit_ta_mod(nn.Module):
    def __init__(self, in_channels, out_channels,T=75):
        ...
        TempA = torch.eye(T).expand((3,T,T)).numpy() # 创建单位矩阵并在第一个维度上复制3次，通过邻接矩阵 TempA 在时序维度上构建关系。
...
class unit_gcn_3d(nn.Module):
    """
    三维的图卷积网络，对输入的时间序列数据进行空间和时间维度上的卷积。
    """
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3, adaptive=True, attention=True, win = 3):
        """A是图的邻接矩阵，A.shape[0]表示多少种不同的图"""
        super(unit_gcn_3d,self).__init__()

        self.win = win # 时间窗口的大小，结合时间和空间地信息
        self.num = A.shape[0]

        def build_spatial_temporal_graph(A_binary, window_size):
            """
            用于生成一个时空图，将图的空间维度与时间窗口结合在一起，生成一个大图
            :param A_binary:输入的二值邻接矩阵
            :param window_size:时间窗口大小
            """
            assert isinstance(A_binary, np.ndarray), 'A_binary should be of type `np.ndarray`'
            V = len(A_binary) # 节点数量
            V_large = V * window_size # 希望将windows_size的时间步的信息在一起卷积
            A_large = np.tile(A_binary, (window_size, window_size)).copy() # 让A_binary这个矩阵分别在行和列上重复windows_size次，变成一个大矩阵
            return A_large

        A_tile = [build_spatial_temporal_graph(A[i],self.win) for i in range(self.num)] # A_tile是一个列表，包含所有扩展后的邻接矩阵
        self.A_tile = torch.Tensor(A_tile).numpy()
        self.gcn = unit_gcn_qkv(in_channels,out_channels,self.A_tile,adaptive=adaptive,attention=attention) # 构建图卷积层实现自注意力机制
