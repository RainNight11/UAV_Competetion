...
class UavDataset(Dataset): # 构建数据集
    def __init__(self, data_path, label_path=None,
                ...
                window_size=-1, normalization=False, debug=False, use_mmap=True,is_test=False,normal_center=False,random_rot=False,pre_normalization=False,
                 zaxis=None, xaxis=None):
        """
        ...
        # # 调用对齐函数
        # if zaxis:
        #     # print('Aligning the bone between joint %s and joint %s to the z-axis.' % (zaxis[0], zaxis[1]))
        #     preprocess.align_human_to_vector(zaxis[0], zaxis[1], [0, 0, 1])
        # if xaxis:
        #     # print('Aligning the bone between joint %s and joint %s to the x-axis.' % (xaxis[0], xaxis[1]))
        #     preprocess.align_human_to_vector(xaxis[0], xaxis[1], [1, 0, 0])
    ...
    def __getitem__(self, index):
        ...
        #     data_numpy = preprocess.pre_normalization(data_numpy)
